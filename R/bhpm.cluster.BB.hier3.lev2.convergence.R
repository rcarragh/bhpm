# bhpm.cluster.BB.dep.lev2.convergence.diag
# Model bhpm.BB
# R. Carragher
# Date: 29/06/2018
#
...
Id <- "$Id: bhpm.cluster.BB.hier3.lev2.convergence.R,v 1.10 2019/06/02 14:12:12 clb13102 Exp clb13102 $"

bhpm.cluster.BB.dep.lev2.convergence.diag <- function(raw, debug_diagnostic = FALSE)
{
	c_base = bhpm.cluster.1a.dep.lev2.convergence.diag(raw, debug_diagnostic)
				...
				for (j in 1:raw$nOutcome[i, b]) {
					for (c in 1:nchains) {
						for (t in 1:(raw$nTreatments - 1)) {
							...
							row <- data.frame(Trt.Grp = theta.trt.grps[t], chain = c, Cluster = raw$Clusters[i], Outcome.Grp = raw$Outcome.Grp[i, b],
...
bhpm.cluster.BB.dep.lev2.print.convergence.summary <- function(conv) {
	if (is.null(conv)) {
