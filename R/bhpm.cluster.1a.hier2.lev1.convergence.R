# bhpm.cluster.1a.hier2.lev1.convergence.diag
# Model bhpm.BB
# R. Carragher
# Date: 29/06/2018
#
...
Id <- "$Id: bhpm.cluster.1a.hier2.lev1.convergence.R,v 1.9 2019/06/02 14:12:13 clb13102 Exp clb13102 $"

bhpm.cluster.1a.hier2.lev1.convergence.diag <- function(raw, debug_diagnostic = FALSE)
{
	if (is.null(raw)) {
		print("NULL raw data")
		return(NULL)
	...
	mu.gamma_conv = data.frame(Outcome.Grp = character(0),
						stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	mu.theta_conv = data.frame(Trt.Grp = integer(0), Outcome.Grp = character(0),
						stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	sigma2.gamma_conv = data.frame(Outcome.Grp = character(0),
						stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	sigma2.theta_conv = data.frame(Trt.Grp = integer(0), Outcome.Grp = character(0),
						stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	...
	if (nchains > 1) {
		...
		for (b in 1:raw$nOutcome.Grp[i]) {
			...
			if (mu.gamma_mon == 1) {
				...
				row <- data.frame(Outcome.Grp = raw$Outcome.Grp[i, b],
			...
			if (mu.theta_mon == 1) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$GelmanRubin(raw$mu.theta[, t, b, ], nchains)
					row <- data.frame(Trt.Grp = theta.trt.grps[t], Outcome.Grp = raw$Outcome.Grp[i, b],
										stat = g$psrf[1], upper_ci =  g$psrf[2], stringsAsFactors=FALSE)
			...
			if (sigma2.theta_mon == 1) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$GelmanRubin(raw$sigma2.theta[, t, b, ], nchains)
					row <- data.frame(Trt.Grp = theta.trt.grps[t], Outcome.Grp = raw$Outcome.Grp[i, b],
										stat = g$psrf[1], upper_ci =  g$psrf[2], stringsAsFactors=FALSE)
		...
		for (b in 1:raw$nOutcome.Grp[i]) {
			...
			if (mu.gamma_mon == 1) {
				...
				row <- data.frame(Outcome.Grp = raw$Outcome.Grp[i, b], stat = g$z, upper_ci = NA, stringsAsFactors=FALSE)
				mu.gamma_conv = rbind(mu.gamma_conv, row)
			}
			...
			if (mu.theta_mon == 1) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$Geweke(raw$mu.theta[1, t, b, ])
			...
			if (sigma2.theta_mon == 1) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$Geweke(raw$sigma2.theta[1, t, b, ])
					row <- data.frame(Trt.Grp = theta.trt.grps[t], Outcome.Grp = raw$Outcome.Grp[i, b], stat = g$z, upper_ci = NA, stringsAsFactors=FALSE)
					sigma2.theta_conv = rbind(sigma2.theta_conv, row)
				}
			}
...
bhpm.cluster.1a.hier2.lev1.print.convergence.summary <- function(conv) {
	...
	if (conv$type == "Gelman-Rubin") {
		if (theta_mon == 1) {
			...
			max_t = head(conv$theta.conv.diag[conv$theta.conv.diag$stat ==
						max(conv$theta.conv.diag$stat),,
						drop = FALSE], 1)
			...
			min_t = head(conv$theta.conv.diag[conv$theta.conv.diag$stat ==
						min(conv$theta.conv.diag$stat),,
						drop = FALSE], 1)
			cat(sprintf("Min Gelman-Rubin diagnostic (%d %s, %s %s): %0.6f\n",
						max_t$Trt.Grp, min_t$Cluster, min_t$Outcome.Grp, min_t$Outcome, min_t$stat))
		}
