...
Id <- "$Id: bhpm.cluster.BB.hier3.lev1.summary.stats.R,v 1.10 2019/06/02 14:12:12 clb13102 Exp clb13102 $"

bhpm.cluster.BB.dep.lev1.summary.stats <- function(raw, prob = 0.95)
{
	s_base = bhpm.cluster.1a.dep.lev1.summary.stats(raw, prob)
	...
	pi_summ = data.frame(Trt.Grp = integer(0), Outcome.Grp = character(0), mean = numeric(0), median = numeric(0),
	...
	if (pi_mon == 1) {
		for (b in 1:raw$nOutcome.Grp[i]) {
			for (t in 1:(raw$nTreatments -1 )) {
				s = M_global$summaryStats(raw$pi[, t, b, ], nchains, prob)
...
bhpm.cluster.BB.dep.lev1.print.summary.stats <- function(summ)
{
	if (is.null(summ)) {
	...
	if (pi_mon == 1)
		for (i in 1:nrow(summ$pi.summary)) {
			...
			cat(sprintf("pi[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n",
						row$Trt.Grp, row$Outcome.Grp, row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	if (mu.gamma.0_mon == 1)
