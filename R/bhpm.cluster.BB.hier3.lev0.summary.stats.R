...
Id <- "$Id: bhpm.cluster.BB.hier3.lev0.summary.stats.R,v 1.11 2019/06/02 14:12:12 clb13102 Exp clb13102 $"

bhpm.cluster.BB.indep.summary.stats <- function(raw, prob = 0.95)
{
	s_base = bhpm.cluster.1a.indep.summary.stats(raw, prob)
	...
	alpha.pi_summ = data.frame(Trt.Grp = integer(0), Cluster = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	beta.pi_summ = data.frame(Trt.Grp = integer(0), Cluster = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	...
	for (i in 1:raw$nClusters) {
		if (alpha_pi_mon == 1) {
			for (t in 1:(raw$nTreatments - 1)) {
				s = M_global$summaryStats(raw$alpha.pi[, t, i, ], nchains, prob)
				row <- data.frame(Trt.Grp = theta.trt.grps[t], Cluster = raw$Clusters[i], mean = s[1], median = s[2],
		...
		if (beta_pi_mon == 1) {
			for (t in 1:(raw$nTreatments - 1)) {
				s = M_global$summaryStats(raw$beta.pi[, t, i, ], nchains, prob)
				row <- data.frame(Trt.Grp = theta.trt.grps[t], Cluster = raw$Clusters[i], mean = s[1], median = s[2],
						hpi_lower = s[3], hpi_upper = s[4],
						SD = s[5], SE = s[6])
...
bhpm.cluster.BB.indep.print.summary.stats <- function(summ)
{
	if (is.null(summ)) {
	...
	if (mu.theta.0_mon == 1) {
		for (i in 1:nrow(summ$mu.theta.0.summary)) {
			...
			cat(sprintf("mu.theta.0[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n", row$Trt.Grp, row$Cluster,
						row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	}
	...
	if (tau2.theta.0_mon == 1) {
		for (i in 1:nrow(summ$tau2.theta.0.summary)) {
			...
			cat(sprintf("tau2.theta.0[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n", row$Trt.Grp, row$Cluster,
						row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	}
	if (alpha_pi_mon == 1) {
		for (i in 1:nrow(summ$alpha.pi.summary)) {
			...
			cat(sprintf("alpha.pi[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n", row$Trt.Grp, row$Cluster,
						row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	}
	if (beta_pi_mon == 1) {
		for (i in 1:nrow(summ$beta.pi.summary)) {
			...
			cat(sprintf("beta.pi[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n", row$Trt.Grp, row$Cluster,
						row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	}
}
