...
Id <- "$Id: bhpm.cluster.1a.hier3.lev2.summary.stats.R,v 1.11 2019/06/02 14:12:12 clb13102 Exp clb13102 $"

bhpm.cluster.1a.dep.lev2.summary.stats <- function(raw, prob = 0.95)
{
	if (is.null(raw)) {
		print("NULL raw data");
		return(NULL)
	...
	if (mu.gamma.0_mon == 1) {
		...
		row <- data.frame(mean = s[1], median = s[2], hpi_lower = s[3], hpi_upper = s[4],
					SD = s[5], SE = s[6])
...
bhpm.cluster.1a.dep.lev2.print.summary.stats <- function(summ)
{
	if (is.null(summ)) {
