# bhpm.cluster.BB.indep.convergence.diag
# Model bhpm.BB
# R. Carragher
# Date: 29/06/2018
#
...
Id <- "$Id: bhpm.cluster.BB.hier3.lev0.convergence.R,v 1.9 2019/06/02 14:12:12 clb13102 Exp clb13102 $"

bhpm.cluster.BB.indep.convergence.diag <- function(raw, debug_diagnostic = FALSE)
{
	c_base = bhpm.cluster.1a.indep.convergence.diag(raw, debug_diagnostic)
	...
	alpha.pi_conv = data.frame(Trt.Grp = integer(0), Cluster = character(0), stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	beta.pi_conv = data.frame(Trt.Grp = integer(0), Cluster = character(0), stat = numeric(0), upper_ci = numeric(0), stringsAsFactors=FALSE)
	...
	if (nchains > 1) {
		...
		if (alpha_pi_mon == 1) {
			for (i in 1:raw$nClusters) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$GelmanRubin(raw$alpha.pi[, t, i, ], nchains)
		...
		if (beta_pi_mon == 1) {
			for (i in 1:raw$nClusters) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$GelmanRubin(raw$beta.pi[, t, i, ], nchains)
					row <- data.frame(Trt.Grp = theta.trt.grps[t], Cluster = raw$Clusters[i], stat = g$psrf[1], upper_ci =  g$psrf[2], stringsAsFactors=FALSE)
		...
		for (i in 1:raw$nClusters) {
			...
			if (alpha_pi_mon == 1) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$Geweke(raw$alpha.pi[1, t, i, ])
			...
			if (beta_pi_mon == 1) {
				for (t in 1:(raw$nTreatments - 1)) {
					g = M_global$Geweke(raw$beta.pi[1, t, i, ])
					row <- data.frame(Trt.Grp = theta.trt.grps[t], Cluster = raw$Clusters[i], stat = g$z, upper_ci = NA, stringsAsFactors=FALSE)
					beta.pi_conv = rbind(beta.pi_conv, row)
				}
			}
	...
	alpha.pi_acc = data.frame(chain = numeric(0), Trt.Grp = integer(0), Cluster = character(0), rate = numeric(0), stringsAsFactors=FALSE)
	beta.pi_acc = data.frame(chain = numeric(0), Trt.Grp = integer(0), Cluster = character(0), rate = numeric(0), stringsAsFactors=FALSE)
	...
	if (raw$sim_type == "MH") {
		for (i in 1:raw$nClusters) {
			for (c in 1:nchains) {
				if (alpha_pi_mon == 1) {
					for (t in 1:(raw$nTreatments - 1)) {
						rate <- raw$alpha.pi_acc[c, t, i]/raw$iter
						row <- data.frame(chain = c, Trt.Grp = theta.trt.grps[t], Cluster = raw$Clusters[i], rate = rate, stringsAsFactors=FALSE)
						alpha.pi_acc = rbind(alpha.pi_acc, row)
					}
				}
				if (beta_pi_mon == 1) {
					for (t in 1:(raw$nTreatments - 1)) {
						rate <- raw$beta.pi_acc[c, t, i]/raw$iter
						row <- data.frame(chain = c, Trt.Grp = theta.trt.grps[t], Cluster = raw$Clusters[i], rate = rate, stringsAsFactors=FALSE)
...
bhpm.cluster.BB.indep.print.convergence.summary <- function(conv) {
