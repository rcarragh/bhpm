...
bhpm.cluster.1a.indep.summary.stats <- function(raw, prob = 0.95)
{
	...
	sigma2.theta_summ = data.frame(Trt.Grp = integer(0), Cluster = character(0), Outcome.Grp = character(0), mean = numeric(0),
													median = numeric(0), hpi_lower = numeric(0),
													hpi_upper = numeric(0), SD = numeric(0), SE = numeric(0))
	mu.gamma.0_summ = data.frame(Cluster = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	mu.theta.0_summ = data.frame(Trt.Grp = integer(0), Cluster = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	tau2.theta.0_summ = data.frame(Trt.Grp = integer(0), Cluster = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	tau2.gamma.0_summ = data.frame(Cluster = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	...
	for (i in 1:raw$nClusters) {
		# mu.gamma.0
		if (mu.gamma.0_mon == 1) {
			s = M_global$summaryStats(raw$mu.gamma.0[, i, ], nchains, prob)
			row <- data.frame(Cluster = raw$Clusters[i], mean = s[1], median = s[2],
		...
		if (mu.theta.0_mon == 1) {
			for (t in 1:(raw$nTreatments - 1)) {
				s = M_global$summaryStats(raw$mu.theta.0[, t, i, ], nchains, prob)
		...
		if (tau2.gamma.0_mon == 1) {
			s = M_global$summaryStats(raw$tau2.gamma.0[, i, ], nchains, prob)
			row <- data.frame(Cluster = raw$Clusters[i], mean = s[1], median = s[2],
							hpi_lower = s[3], hpi_upper = s[4],
							SD = s[5], SE = s[6])
		...
		if (tau2.theta.0_mon == 1) {
			for (t in 1:(raw$nTreatments - 1)) {
				s = M_global$summaryStats(raw$tau2.theta.0[, t, i, ], nchains, prob)
				row <- data.frame(Trt.Grp = theta.trt.grps[t], Cluster = raw$Clusters[i], mean = s[1], median = s[2],
							hpi_lower = s[3], hpi_upper = s[4],
							SD = s[5], SE = s[6])
