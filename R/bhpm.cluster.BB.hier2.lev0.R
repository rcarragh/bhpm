...
Mi_BB_h2_l0$Id <- "$Id: bhpm.cluster.BB.hier2.lev0.R,v 1.12 2019/05/30 13:32:26 clb13102 Exp clb13102 $"

bhpm.cluster.BB.hier2.lev0 <- function(cluster.data, sim_type = "SLICE", burnin = 10000, iter = 60000, nchains = 5,
	theta_algorithm = "MH",
...
{
	cluster = M_global$CLUSTERdata(Mi_BB_h2_l0, cluster.data, iter, nchains, burnin, initial_values)
	...
	sp = M_global$CLUSTER_sim_paramsBB_2(Mi_BB_h2_l0, sim.params, pm.weights, sim_type, cluster.data, cntrl.data)
	...
	Ret2 = .Call("bhpmBB_cluster_hier2_exec", as.integer(nchains), as.integer(burnin),
					...
					pm.weights,
					monitor,
					as.integer(Mi_BB_h2_l0$nTreatments),
					as.integer(Mi_BB_h2_l0$numClusters),
					as.integer(Mi_BB_h2_l0$Level),
					Mi_BB_h2_l0$maxOutcome.Grps,
					as.integer(Mi_BB_h2_l0$numOutcome.Grp),
					as.integer(Mi_BB_h2_l0$maxOutcomes),
					as.integer(t(Mi_BB_h2_l0$nOutcome)),
	...
	model_fit = list(id = Mi_BB_h2_l0$Id, sim_type = Mi_BB_h2_l0$sim_type, chains = nchains, nClusters = Mi_BB_h2_l0$numClusters,
			nTreatments = Mi_BB_h2_l0$nTreatments,
			Clusters = Mi_BB_h2_l0$Clusters, Trt.Grps = Mi_BB_h2_l0$Trt.Grps, nOutcome.Grp = Mi_BB_h2_l0$numOutcome.Grp,
			maxOutcome.Grps = Mi_BB_h2_l0$maxOutcome.Grps,
			maxOutcomes = Mi_BB_h2_l0$maxOutcomes, nOutcome = Mi_BB_h2_l0$nOutcome, Outcome=Mi_BB_h2_l0$Outcome,
			Outcome.Grp = Mi_BB_h2_l0$Outcome.Grp,
...
Mi_BB_h2_l0$initVars = function() {
    ...
    # Data Structure
    Mi_BB_h2_l0$Outcome.Grp <- c()
    Mi_BB_h2_l0$numOutcome.Grp <- NA
    Mi_BB_h2_l0$numClusters <- NA
    Mi_BB_h2_l0$nOutcome <- c()
    Mi_BB_h2_l0$maxOutcomes <- NA
...
Mi_BB_h2_l0$initChains = function(c) {
	...
	for (i in 1:Mi_BB_h2_l0$numClusters) {
		numOutcome.Grp = Mi_BB_h2_l0$numOutcome.Grp[i]
		for (b in 1:numOutcome.Grp) {
			Mi_BB_h2_l0$gamma[c, i, b, 1:Mi_BB_h2_l0$nOutcome[i, b]] <- runif(Mi_BB_h2_l0$nOutcome[i, b], -10, 10)
			...
			for (t in 1:(Mi_BB_h2_l0$nTreatments -1)) {
				Mi_BB_h2_l0$theta[c, t, i, b, 1:Mi_BB_h2_l0$nOutcome[i, b]] <- runif(Mi_BB_h2_l0$nOutcome[i, b], -10, 10)

				Mi_BB_h2_l0$theta[c, t, i, b, ][is.infinite(Mi_BB_h2_l0$theta[c, t, i, b, ])] = -10

				Mi_BB_h2_l0$theta[c, t, i, b, ][is.nan(Mi_BB_h2_l0$theta[c, t, i, b, ])] = -10 # -1000
			}
		}

		Mi_BB_h2_l0$mu.gamma[c, i, 1:numOutcome.Grp] = runif(numOutcome.Grp, -10, 10)
		Mi_BB_h2_l0$mu.theta[c, , i, 1:numOutcome.Grp] = runif(numOutcome.Grp*(Mi_BB_h2_l0$nTreatments - 1), -10, 10)
		Mi_BB_h2_l0$sigma2.gamma[c, i, 1:numOutcome.Grp] = runif(numOutcome.Grp, 5, 20)
		Mi_BB_h2_l0$sigma2.theta[c, , i, 1:numOutcome.Grp] = runif(numOutcome.Grp*(Mi_BB_h2_l0$nTreatments - 1), 5, 20)

		Mi_BB_h2_l0$pi[c, , i, 1:numOutcome.Grp] = runif(numOutcome.Grp*(Mi_BB_h2_l0$nTreatments - 1), 0, 1)
	}
}

Mi_BB_h2_l0$initialiseChains = function(initial_values, nchains) {

	Mi_BB_h2_l0$theta = array(0, dim=c(nchains, Mi_BB_h2_l0$nTreatments - 1, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps, Mi_BB_h2_l0$maxOutcomes))
	Mi_BB_h2_l0$gamma = array(0, dim=c(nchains, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps, Mi_BB_h2_l0$maxOutcomes))
	...
	if (is.null(initial_values)) {
		...
		for (i in 1:Mi_BB_h2_l0$numClusters) {
			...
			for (b in 1:numOutcome.Grp) {
				...
				for (t in 1:(Mi_BB_h2_l0$nTreatments -1)) {
					Mi_BB_h2_l0$theta[1, t, i, b, ] <- log(Mi_BB_h2_l0$y[t ,i, b,]/Mi_BB_h2_l0$T[t, i, b,]) - Mi_BB_h2_l0$gamma[1, i, b, ]
					Mi_BB_h2_l0$theta[1, t, i, b, ][is.infinite(Mi_BB_h2_l0$theta[1, t, i, b, ])] = -10 # -1000
					Mi_BB_h2_l0$theta[1, t, i, b, ][is.nan(Mi_BB_h2_l0$theta[1, t, i, b, ])] = -10 # -1000
				}
		...
		Mi_BB_h2_l0$mu.gamma <- array(0, dim = c(nchains, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))
		Mi_BB_h2_l0$mu.theta <- array(0, dim = c(nchains, Mi_BB_h2_l0$nTreatments - 1, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))
		Mi_BB_h2_l0$sigma2.gamma <- array(10, dim = c(nchains, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))
		Mi_BB_h2_l0$sigma2.theta <- array(10, dim = c(nchains, Mi_BB_h2_l0$nTreatments - 1, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))

		Mi_BB_h2_l0$pi <- array(0.5, dim = c(nchains, Mi_BB_h2_l0$nTreatments - 1, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))
		...
		Mi_BB_h2_l0$mu.gamma <- array(0, dim = c(nchains, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))
		Mi_BB_h2_l0$sigma2.gamma <- array(0, dim = c(nchains, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))
		Mi_BB_h2_l0$mu.theta <- array(0, dim = c(nchains, Mi_BB_h2_l0$nTreatments - 1, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))
		Mi_BB_h2_l0$sigma2.theta <- array(0, dim = c(nchains, Mi_BB_h2_l0$nTreatments - 1, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))

		Mi_BB_h2_l0$pi <- array(0.5, dim = c(nchains, Mi_BB_h2_l0$nTreatments - 1, Mi_BB_h2_l0$numClusters, Mi_BB_h2_l0$maxOutcome.Grps))

		for (c in 1:nchains) {
			for (i in 1:Mi_BB_h2_l0$numClusters) {
				cluster = Mi_BB_h2_l0$Clusters[i]
				for (b in 1:Mi_BB_h2_l0$numOutcome.Grp[i]) {
					data = initial_values$mu.gamma[initial_values$mu.gamma$chain == c &
												initial_values$mu.gamma$Cluster == cluster
												& initial_values$mu.gamma$Outcome.Grp == Mi_BB_h2_l0$Outcome.Grp[1, b],]
					...
					data = initial_values$sigma2.gamma[initial_values$sigma2.gamma$chain == c &
												...
												& initial_values$sigma2.gamma$Outcome.Grp == Mi_BB_h2_l0$Outcome.Grp[1, b],]
					...
					for (t in 1:(Mi_BB_h2_l0$nTreatments -1)) {
						data = initial_values$mu.theta[[t]][initial_values$mu.theta[[t]]$chain == c &
												...
												& initial_values$mu.theta[[t]]$Outcome.Grp == Mi_BB_h2_l0$Outcome.Grp[1, b],]
						Mi_BB_h2_l0$mu.theta[c, t, i, b] = data$value
						data = initial_values$sigma2.theta[[t]][initial_values$sigma2.theta[[t]]$chain == c &
												initial_values$mu.gamma$Cluster == cluster
												& initial_values$sigma2.theta[[t]]$Outcome.Grp == Mi_BB_h2_l0$Outcome.Grp[1, b],]
						Mi_BB_h2_l0$sigma2.theta[c, t, i, b] = data$value

						data = initial_values$pi[[t]][initial_values$pi[[t]]$chain == c &
												initial_values$pi[[t]]$Cluster == cluster
												& initial_values$pi[[t]]$Outcome.Grp == Mi_BB_h2_l0$Outcome.Grp[1, b],]
						Mi_BB_h2_l0$pi[c, t, i, b] = data$value
					}
				}
			}
		}
		...
		for (c in 1:nchains) {
			for (i in 1:Mi_BB_h2_l0$numClusters) {
				cluster = Mi_BB_h2_l0$Clusters[i]
				for (b in 1:Mi_BB_h2_l0$numOutcome.Grp[i]) {
					for (j in 1:Mi_BB_h2_l0$nOutcome[i, b]) {
						ae = Mi_BB_h2_l0$Outcome[i, b, j]
						data = initial_values$gamma[initial_values$gamma$chain == c
										& initial_values$gamma$Cluster == cluster
										& initial_values$gamma$Outcome.Grp == Mi_BB_h2_l0$Outcome.Grp[i, b]
						...
						for (t in 1:(Mi_BB_h2_l0$nTreatments -1)) {
							data = initial_values$theta[[t]][initial_values$theta[[t]]$chain == c
										...
										& initial_values$theta[[t]]$Outcome.Grp == Mi_BB_h2_l0$Outcome.Grp[i, b]
										& initial_values$theta[[t]]$Outcome == ae,]
							Mi_BB_h2_l0$theta[c, t, i, b, j] = data$value
						}
					}
				}
			}
