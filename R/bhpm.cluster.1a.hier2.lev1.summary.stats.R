...
Id <- "$Id: bhpm.cluster.1a.hier2.lev1.summary.stats.R,v 1.11 2019/06/02 14:12:13 clb13102 Exp clb13102 $"

bhpm.cluster.1a.hier2.lev1.summary.stats <- function(raw, prob = 0.95)
{
	if (is.null(raw)) {
		print("NULL raw data");
		return(NULL)
...
bhpm.cluster.1a.hier2.lev1.print.summary.stats <- function(summ)
{
	if (is.null(summ)) {
	...
	if (mu.theta_mon == 1) {
		for (i in 1:nrow(summ$mu.theta.summary)) {
			...
			cat(sprintf("mu.theta[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n",
				row$Trt.Grp, row$Outcome.Grp, row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	}
	if (sigma2.gamma_mon == 1) {
