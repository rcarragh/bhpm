...
Id <- "$Id: bhpm.cluster.1a.hier3.lev1.summary.stats.R,v 1.11 2019/06/02 14:12:12 clb13102 Exp clb13102 $"

bhpm.cluster.1a.dep.lev1.summary.stats <- function(raw, prob = 0.95)
{
	if (is.null(raw)) {
		print("NULL raw data");
		return(NULL)
	...
	theta_summ = data.frame(Trt.Grp = integer(0), Cluster = character(0), Outcome.Grp = character(0), Outcome = character(0),
													mean = numeric(0), median = numeric(0), hpi_lower = numeric(0),
													hpi_upper = numeric(0), SD = numeric(0), SE = numeric(0))
	mu.gamma_summ = data.frame(Outcome.Grp = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	mu.theta_summ = data.frame(Trt.Grp = integer(0), Outcome.Grp = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0),
													hpi_upper = numeric(0), SD = numeric(0), SE = numeric(0))
	sigma2.gamma_summ = data.frame(Outcome.Grp = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0),
													hpi_upper = numeric(0), SD = numeric(0), SE = numeric(0))
	sigma2.theta_summ = data.frame(Trt.Grp = integer(0), Outcome.Grp = character(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0),
													hpi_upper = numeric(0), SD = numeric(0), SE = numeric(0))
	mu.gamma.0_summ = data.frame(mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	mu.theta.0_summ = data.frame(Trt.Grp = integer(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	tau2.theta.0_summ = data.frame(Trt.Grp = integer(0), mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	tau2.gamma.0_summ = data.frame(mean = numeric(0), median = numeric(0),
													hpi_lower = numeric(0), hpi_upper = numeric(0),
													SD = numeric(0), SE = numeric(0))
	...
	for (b in 1:raw$nOutcome.Grp[i]) {
		# mu.gamma
		if (mu.gamma_mon == 1) {
			s = M_global$summaryStats(raw$mu.gamma[, b, ], nchains, prob)
			row <- data.frame(Outcome.Grp = raw$Outcome.Grp[i,b], mean = s[1], median = s[2], hpi_lower = s[3],
		...
		if (mu.theta_mon == 1) {
			for (t in 1:(raw$nTreatments - 1)) {
				s = M_global$summaryStats(raw$mu.theta[, t, b, ], nchains, prob)
				row <- data.frame(Trt.Grp = theta.trt.grps[t], Outcome.Grp = raw$Outcome.Grp[i,b], mean = s[1], median = s[2], hpi_lower = s[3],
						hpi_upper = s[4], SD = s[5], SE = s[6])
		...
		if (sigma2.theta_mon == 1) {
			for (t in 1:(raw$nTreatments - 1)) {
				s = M_global$summaryStats(raw$sigma2.theta[, t, b, ], nchains, prob)
				row <- data.frame(Trt.Grp = theta.trt.grps[t], Outcome.Grp = raw$Outcome.Grp[i,b], mean = s[1], median = s[2], hpi_lower = s[3],
	                       hpi_upper = s[4], SD = s[5], SE = s[6])
		...
		if (sigma2.gamma_mon == 1) {
			s = M_global$summaryStats(raw$sigma2.gamma[, b, ], nchains, prob)
   			row <- data.frame(Outcome.Grp = raw$Outcome.Grp[i,b], mean = s[1], median = s[2], hpi_lower = s[3],
						hpi_upper = s[4], SD = s[5], SE = s[6])
	...
	# mu.gamma.0
	if (mu.gamma.0_mon == 1) {
		s = M_global$summaryStats(raw$mu.gamma.0[,], nchains, prob)
	...
	if (mu.theta.0_mon == 1) {
		for (t in 1:(raw$nTreatments - 1)) {
			s = M_global$summaryStats(raw$mu.theta.0[,t,], nchains, prob)
			row <- data.frame(Trt.Grp = theta.trt.grps[t], mean = s[1], median = s[2],
						hpi_lower = s[3], hpi_upper = s[4],
						SD = s[5], SE = s[6])
	...
	if (tau2.gamma.0_mon == 1) {
		s = M_global$summaryStats(raw$tau2.gamma.0[,], nchains, prob)
		row <- data.frame(mean = s[1], median = s[2],
						hpi_lower = s[3], hpi_upper = s[4],
						SD = s[5], SE = s[6])
	...
	if (tau2.theta.0_mon == 1) {
		for (t in 1:(raw$nTreatments - 1)) {
			s = M_global$summaryStats(raw$tau2.theta.0[,t,], nchains, prob)
			row <- data.frame(Trt.Grp = theta.trt.grps[t], mean = s[1], median = s[2],
						hpi_lower = s[3], hpi_upper = s[4],
						SD = s[5], SE = s[6])
...
bhpm.cluster.1a.dep.lev1.print.summary.stats <- function(summ)
{
	if (is.null(summ)) {
	...
	if (mu.theta_mon == 1) {
		for (i in 1:nrow(summ$mu.theta.summary)) {
			...
			cat(sprintf("mu.theta[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n", row$Trt.Grp,
					row$Outcome.Grp, row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	}
	if (sigma2.gamma_mon == 1) {
	...
	if (sigma2.theta_mon == 1) {
		for (i in 1:nrow(summ$sigma2.theta.summary)) {
			...
			cat(sprintf("sigma2.theta[%d %s]: %0.6f (%0.6f %0.6f) %0.6f %0.6f\n", row$Trt.Grp,
					row$Outcome.Grp, row$mean, row$hpi_lower, row$hpi_upper, row$SD, row$SE))
		}
	}
	if (mu.gamma_mon == 1) {
